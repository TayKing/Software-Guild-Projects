    ...
    public class VehicleRepositoryTest : IVehicleRepository
    {
        ...
        public List<BodyStyle> GetAllBodyStyles()
        {
            if (_BodyStyles == null)
            {
                ...
                _BodyStyles.Add(new BodyStyle() { BodyStyleID = 1, Body = "Car" });
        ...
        public List<Make> GetAllMakes()
        {
            if (_makes == null)
            {
                ...
                _makes.Add(new Make() { MakeID = 1, MakeName = "Ford", UserCreated = "user1", DateCreated = "1-1-2019" });
                _makes.Add(new Make() { MakeID = 2, MakeName = "Toyota", UserCreated = "user2", DateCreated = "2-1-2019" });
                _makes.Add(new Make() { MakeID = 3, MakeName = "Kia", UserCreated = "user3", DateCreated = "3-1-2019" });
            }
            return _makes;
        }
        ...
        public List<Model> GetAllModels()
        {
            if (_models == null)
            {
                ...
                _models.Add(new Model() { ModelID = 1, MakeID = 1, ModelName = "Explorer", UserCreated = "user1", DateCreated = "1-1-2019" });
                _models.Add(new Model() { ModelID = 2, MakeID = 1, ModelName = "Mustang", UserCreated = "user1", DateCreated = "1-1-2019" });
                _models.Add(new Model() { ModelID = 3, MakeID = 2, ModelName = "Prius", UserCreated = "user1", DateCreated = "1-1-2019" });
                _models.Add(new Model() { ModelID = 4, MakeID = 2, ModelName = "Camry", UserCreated = "user1", DateCreated = "1-1-2019" });
                _models.Add(new Model() { ModelID = 5, MakeID = 3, ModelName = "Sorento", UserCreated = "user1", DateCreated = "1-1-2019" });
                _models.Add(new Model() { ModelID = 6, MakeID = 3, ModelName = "Optima", UserCreated = "user1", DateCreated = "1-1-2019" });
        ...
        public List<Special> GetAllSpecials()
        {
            if (_specials == null)
            {
                ...
                _specials.Add(new Special() { SpecialID = 1, SpecialName = "Cash Back", SpecialDescription = "Get cash back on your purchuse!" });
                _specials.Add(new Special() { SpecialID = 2, SpecialName = "Free TV", SpecialDescription = "Free 60in HD 4K TV with vehicle purchuse!" });
                _specials.Add(new Special() { SpecialID = 3, SpecialName = @"0% Intrests Financing", SpecialDescription = "No interest for the first year when you finance with us!" });
            }
            return _specials;
        }
        ...
        public List<State> GetAllStates()
        {
            if (_states == null)
            {
                ...
                _states.Add(new State() { StateID = "KY", StateName = "Kentucky" });
        ...
        public List<InventoryReport> GetInventoryReport(bool SearchNew)
        {
            ...
            if (SearchNew)
                vehicles = GetNewVehicles();
            else
                vehicles = GetUsedVehicles();
            ...
            foreach (VehicleViewModel v in vehicles)
            {
                ...
                {
                    if (r.Make == v.Make && r.VehicleYear == v.VehicleYear && r.Model == v.Model)
                    {
                        r.Count++;
                        r.StockValue += v.Price;
                        needToAdd = false;
                    }
                }
                if (needToAdd)
        ...
        public List<SalesReport> GetSalesReport(string UserID, string StartDate, string EndDate)
        {
            ...
            foreach (Sales p in _Sales)
            {
                ...
                if (!string.IsNullOrEmpty(StartDate))
                    if (DateTime.Parse(StartDate) > DateTime.Parse(p.SalesDate))
                        continue;

                if (!string.IsNullOrEmpty(EndDate))
                    if (DateTime.Parse(EndDate) < DateTime.Parse(p.SalesDate))
                        continue;
